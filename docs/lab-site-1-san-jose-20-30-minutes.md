...
3. From the browser home Page, select **Cisco Webex Links** > **Cisco Webex Control Hub**. Log in with **cholland@cbXXX.dc-YY.com** and **dCloud*AAAA*!.** Accept all security warnings and welcome prompts!

NOTE: You need to Replace **XXX, YY** and **YYYY** with your session-specific details. Credentials for your session are located in text file Session\_Info.txt on Workstation Desktop.

   ![](assets/docx-image-009.png)

1. For security reasons, **Webex Control Hub** signs out every 20 minutes (Idle timeout) by default. For this lab, let’s make the idle time out longer so the Control Hub does not sign you out often during this lab.
2. Go to **MANAGEMENT > Organization Settings > Control Hub’s idle timeout.** Drop down the option for **Control Hub idle timeout** and select **12 hours** or **no timeout**. Click **Save**.

   ![A screenshot of a computer Description automatically generated](assets/docx-image-010.png)

1. Now, go to **MANAGEMENT** > **Locations.** On the locations page you will see some pre-created locations. Select the location **San Jose**.

	![](assets/docx-image-011.png)

1. On San Jose location page go to **PSTN** tab. Select **Configure PSTN Services**.

	![](assets/docx-image-012.png)

1. Click **Manage** for PSTN Configuration > PSTN connection.

	![](assets/docx-image-013.png)

1. It will take you to set up PSTN connection type for the **San Jose**. On the **Edit PSTN connection for San Jose** page keep the option selected for **Cisco Calling Plans (CCP)** and select **Next**.

	![](assets/docx-image-014.png)

1. On the following page populate below information and click **Next**.
...
1. Click **Yes, Change** on the Contract Information Update pop-up window.
2. On the **Emergency disclaimer** page scroll all the down and enter following information and click **Agree and Continue**.
   * Authorized Contact: Charles Holland
   * Job title: Engineer
3. On the **Emergency Services Address**, leave all fields default and click **Save**.
4. Click **Apply** on the address verification pop-up.
5. Click **Save** (again) or **Done.**
6. Now the location **San Jose** has been setup to **Cisco Calling Plans** PSTN Connection type.
7. Click **Add Numbers**

	![](assets/docx-image-016.png)

1. On the **Add Numbers** page, keep **Order New Numbers** selected and click **Next**
2. On the next page, keep the **State/Province/Region** selected for **California**. Drop down the option for **Area Code** and choose any of the area codes available. For How many numbers do you want to auto-selected for you? enter ***3*** and click **Search.**

	![](assets/docx-image-017.png)

1. On the next page, Control Hub will select 3 numbers for you. Click **Order**.
2. It will order 3 new numbers for the location San Jose. Click **View orders**.

	![](assets/docx-image-018.png)

1. It will take you to **PSTN & Routing** > **PSTN orders** tab/page, click on the order you just placed, and observe the order status, it is should say **Provisioned.** If not, wait 1 minute and try again.

	![](assets/docx-image-019.png)

1. Now on the PSTN & Routing page, go to **Numbers** tab, and observe the three newly placed +E.164 numbers, make sure the Status for all numbers show as **Active**.
...
1. On Edit services for [cholland@cbXXX.dc-YY.com](mailto:cholland@cbXXX.dc-YY.com) page, observe that no “Webex calling professional” license is assigned. Click the **Edit Licenses** button
...
1. On the next page, drop down the option for **Location** and choose **San Jose**. Now drop down the option for **Phone Number** and choose any +E.164 phone number. For **Extension** enter last 4 digits of the assigned DID number. **Make a note of Charles 4 digit extension number, you will need it later!** Click Save. Click **Close**.

	![](assets/docx-image-026.png)

1. Repeat all the above steps for **Anita Perez** user. Make sure you assign last ***five*** digits of the DID number as extension number. Make a note of this number, you will need it later.
...
1. Open RDP connection to Workstation 2, either using **Option A** or **Option B** described in Accessing your Lab section.

* Workstation IP Address: **198.18.1.37**
* Username: **dcloud\aperez**
* Password: **dCloud123!**

1. Once logged into Workstation 2, bring up Webex and login as [aperez@cbXXX.dc-YY.com](mailto:aperez@cbXXX.dc-YY.com) & dCloudAAAA!. **NOTE:** Make sure you replace XXX, YY and AAAA values with your session specific details located in text file called Session\_Info.txt on ***Workstation 2*** Desktop.
...
1. Continuing on workstation 1, Webex Control Hub.

Before we configure the Routing Prefix for each location, we need to define some parameters like how long is each Routing Prefix, and the digit each Routing Prefix starts with etc.

1. On Webex Control Hub, go to **SERVICES** > **Calling**. On the Calling page go to **Settings** tab.
2. On the **Settings** page click **Edit** for **Internal Dialing**.

	![](assets/docx-image-033.png)

1. Click **Continue** on **Edit Internal Dialing** pop-up window to confirm that phones need to be restarted for the changes to take effect. On the following page, enter below values and click **Save**.
...
1. It will take you back to Settings page, leave **Allow extension dialing between locations** field (under Internal Dialing) Toggled ON. This setting controls if a user can dial between sites without using a site code, with preference being given to a local site match. We will look at this parameter in detail later! For now just make sure its toggled ON.
...
![A white rectangle with blue and orange border

Description automatically generated](assets/docx-image-036.png)

1. Now let's define the **Routing Prefix** for **San Jose** location. On **Webex Control Hub** go to **MANAGEMENT** > **Locations**.
2. On the locations page select location **San Jose**. On San Jose location page go to **Calling** tab. On the Calling tab go to **Dialing** > **Internal dialing**.  
   ![](assets/docx-image-037.png)
3. Configure **Routing prefix** as **42**. Observe the dialing preview changes to reflect 42-XXXX and click **Save**.
