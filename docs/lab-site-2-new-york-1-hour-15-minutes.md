...
1. On **New York** location page go to **PSTN** tab. Select **Configure PSTN Services**.

![](assets/docx-image-046.png)

1. On the next page, click **Manage** for PSTN Configuration > PSTN connection.

![](assets/docx-image-047.png)

1. It will take you to set up PSTN connection type for the **New York**. On the **Edit PSTN connection for New York** page select option Cloud Connected PSTN and click **Next**.

![](assets/docx-image-048.png)

1. On the next page, for **Select a Provider**, choose the Intelepeer provider, and click **Next.**

![](assets/docx-image-049.png)

1. On the next page for **Emergency Services Address**, keep all default and click **Save**. If prompted click **Apply** for suggested address, then click **Save** again.
2. On the next page, click **Add Numbers Now**
3. On the next page, leave the Location as **New York** and Number type as **PSTN number** and click **Next.**

![](assets/docx-image-050.png)

1. On the next page populate two 10 digit phone numbers given/allocated to you by your proctor. Click **Save**.

![](assets/docx-image-051.png)

**NOTE**: If you have not yet got assigned phone numbers for this module reach out to of the proctors, they will help you.

1. On the next page click **View numbers,** it will take you to **SERVICES** > **PSTN & Routing** > **Numbers** page, ensure both numbers are added and show the status **Active**.
...
1. It will open a PowerShell window and execute the script. Once the script is executed the script window will close automatically and a configuration file will be created on the Workstation 1 Desktop called **InternalcallsLGW\_Config-Ready.txt**.
2. Double click and open the file.
3. Now, open Putty from task bar and connect to the Local Gateway at 198.18.133.227 over SSH, using credentials - username: **admin** and password: **dCloud123!**
...
##### 527. Wait for 2 minutes, then run the following command on Putty window to confirm the TLS session from Local Gateway to Webex Cloud.

show sip-ua connection tcp tls detail

![](assets/docx-image-057.png)

1. You should see an active TLS connection established. The 150.X.X.X subnet belongs to Webex.
2. Now issue the following command, and check the registration is shown as ***yes***  
     
   show sip-ua register status  
     
   ![](assets/docx-image-058.png)
3. Now open control hub, navigate to PSTN & Routing, Gateway Configurations and then Trunk. Notice that the IntCal trunk is now showing online, because we completed the registration on the Local Gateway.  
...
This completes the Webex Calling side config of the trunk for internal calls. Note that we used DPG matching in our scripts here, but we do not use DPG Matching on dial peers with Webex Calling Survivability active. Bear this in mind if configuring for your customers! Check the [configuration guide](https://help.webex.com/en-us/article/w0qmeu/Colocation-of-Local-Gateway-and-Site-Survivability-on-Cisco-IOS-Managed-Gateways) for more information.
...
1. On the next page, drop down the option for **Location** and choose **New York**. Now drop down the option for **Phone Number** and choose one of the available phone numbers. Make sure you assign last ***four*** digits of the DID number as extension number.

![](assets/docx-image-068.png)

1. Once the user is assigned the DID number, navigate to SERVICES > PSTN & Routing. On the Numbers page observe that New York number would not have site routing code. We will configure in the next module.
...
1. Now click on line 1 – 6026 in Base\_PT  
   ![](assets/docx-image-075.png)
2. Adjust the directory number to match the full +E.164 you have for this site in control hub. Ensure you prefix it with a \ character. Tab out of the field, and allow the page to refresh.  
   ![](assets/docx-image-076.png)
3. Change all fields on the page that specify Taylor Bard to read Kellie Melby, and save the config.  
   ![](assets/docx-image-077.png)
4. Next, add click the “**Add** **Enterprise Alternate Number**” button and use a number mask of 42**XXXX** – Where XXXX is the last four digits of **Charles Hollands number.** We are configuring a simulated extension number overlap here between a UCM User and a Webex Calling user, which we will test later. Remember to set your Route Partition too!  
   ![](assets/docx-image-078.png)
5. Now select User Management > End user.  
   ![](assets/docx-image-079.png)
6. Search for Kellie and open her profile. Click on the Device Association button.  
   ![](assets/docx-image-080.png)
7. Search for the device, tick the box next to it and save the changes.  
   ![](assets/docx-image-081.png)
8. In the top right of the screen, click the Go button next to **Back to user**![](assets/docx-image-082.png)
9. Scroll down on the page, set the primary extension and then save the end user configuration  
   ![](assets/docx-image-083.png)
10. Kellie’s Jabber configuration is now complete! Next, we must configure Webex Calling & UCM to trunk calls to her number through the trunk we created earlier; Both inbound and outbound. The users existing calling search space routes their PSTN calls out through the regular CUBE Route list. We need to send calls out through the Webex Calling Local Gateway Trunk. Click on **Call Routing > Class Of Control > Partition**![](assets/docx-image-084.png)
11. Add a new partition as follows  
    P\_CloudPSTN, Partition for Cloud PSTN Calls  
    ![](assets/docx-image-085.png)
12. Click on **Call Routing > Class Of Control > Calling Search Space** and add a new one with the following properties. You’ll need to scroll down on the Available Partitions list. Click **Save**  
    ![](assets/docx-image-086.png)
13. Open **Device > Phone** and open the **csfkmelby** device  
    ![](assets/docx-image-087.png)
14. Set the Phone **Calling Search Space** to **CSS-CloudPSTN**![](assets/docx-image-088.png)
15. Click on Line 1 in the top left  
    ![](assets/docx-image-089.png)
16. Adjust the phone **Calling Search Space** to **CSS-CloudPSTN** and click save![](assets/docx-image-090.png)
17. Click on **Call Routing > Route/Hunt > Route Pattern**![](assets/docx-image-091.png)
18. We need to create a route pattern to point PSTN Calls over the Webex Calling Trunk. Copy the route pattern for **OUTBOUND DIALLING – US** – The copy button is on the far right  
    ![](assets/docx-image-092.png)
19. Adjust the parameters as follows ensuring the highlighted areas match the image. Click Save and OK on the subsequent two screens.  
    ![](assets/docx-image-093.png)
20. Now, we need to configure control hub to send calls inbound from Cloud PSTN over the trunk to UCM to route to Kellie. From control hub, Navigate to **PSTN & Routing > Gateway Configurations > Route Lists**. Click on **Create Route List**![](assets/docx-image-094.png)
21. Add your route list with the following configuration settings, and click Create  
    ![](assets/docx-image-095.png)
22. You will be presented with a screen to add PSTN DID’s to the route list. Select the one that you have available – This tells Webex Calling to send this number over the trunk to UCM. You can add up to 100 numbers at a time manually, and it is also possible to add them in bulk via CSV. **Add your number and click Add**  
...
1. It will take you to **Add virtual line** page. Populate the following and click **Add**.
...
1. It will create the new virtual line, now we need to assign this line to a user. Click **Assign**.
2. On the next page click **Assign device**, it will bring Select device drop down option. Drop down the option and choose **Eric Steele**. Click **Assign.**

![](assets/docx-image-101.png)

1. On the next page, keep all default values and click **Save**. Observe that you have options to configure the line layout, line Label and call decline policy etc.
...
1. On the next page, keep the radio button selected for Organization and populate the following values and click **Create**.
...
2. On the next screen, enter the username **kmelby** and password **dCloud123!**![](assets/docx-image-106.png)
...
\*Sep 10 08:54:17.587: //4859/36AE4F000000/SIP/Msg/ccsipDisplayMsg:
...
1. This completes the testing for our outbound call leg from UCM, through Webex Calling and out to our Cloud PSTN Provider. Now lets test inbound. Issue the show run command and scroll using space bar until you see the configuration that looks something like the below. The red part will change in your deployment. This command identifies the trunk we configured earlier.

voice class uri 301 sip

pattern dtg=**intcal2112036796\_lgu**

1. Make a call from your cellphone to Kellie’s PSTN number, and check the debugs on the gateway again. Notice the highlighted area in red – this identifies the trunk the call was received on.  
     
   \*Sep 10 09:00:19.085: //-1/xxxxxxxxxxxx/SIP/Msg/ccsipDisplayMsg:

Received:

INVITE sip:+17206470845@198.18.1.227:5070;transport=tls;**dtg=intcal2112036796\_lgu** SIP/2.0
...
5. Open **Locations > New York** then click on **Calling > Internal Dialing**![](assets/docx-image-110.png)
...
9. Configure it as follows and click **Save**![](assets/docx-image-113.png)
10. Click on **Call Routing > Class Of Control > Calling Search Space** and add a new one.![](assets/docx-image-114.png)
...
3. Open **Call Routing > Route Plan Report**![](assets/docx-image-125.png)
...
11. Next, assign the only free number from the New York location – And remember **Give Kellie the same 4 digit directory number as Charles Holland!** Then click **Save**![](assets/docx-image-133.png)
12. Open Control Hub and Go to **Calling > Settings** and re-enable **Allow Extension Dialling Between Locations** – We are returning this parameter to the default setting before testing further.![](assets/docx-image-134.png)
...
Description automatically generated](assets/docx-image-154.png)
...
6. The length of unknown extensions can be anything from 2-10 digits. It is important to set this accurately, since some countries such as the USA have nonspecific PSTN Dial plans that route 7 digits to PSTN, such as XXXXXXX. Using the example of Germany, any number from 5 digits in length, all the way up to 13 digits in length constitutes a valid destination for PSTN routing! So how do we route to internal unknown extension numbers in that scenario? With what we just learned, it would seem that all of our internal users would be pushed to PSTN instead of over the trunk to UCM! See a screenshot of the Germany PSTN dial plan below.  
   ...
   Description automatically generated](assets/docx-image-159.png)
...
11. Set the outbound dial digit to 9 and enable enforcement. Repeat this for the New York site. We cannot set this for Dallas & London yet, as we have not configured calling for them.![](assets/docx-image-161.png)
...
1. Open PSTN & Routing, Gateway Configurations, Verify Call Routing, Choose Charles Holland as the source and enter the 7-digit number 4416026. Note it now routes internally as an unknown extension, since our maximum unknown extension length is 8 digits, and we didn’t dial the outbound dial digit of 9 to indicate it’s a PSTN Call  
   ![](assets/docx-image-162.png)
2. Check again with a 9 at the front. Note the call goes to PSTN now!  
   ![](assets/docx-image-163.png)
3. Let’s step through our destination selection scenario again to understand this process.

![A diagram of a selection

Description automatically generated](assets/docx-image-164.png)
...
1. We have now set up our UCM-Webex Calling internal calls trunk. In practice, remember to consider the interactions between maximum unknown extension length, the default PSTN dial plan, and enforce outbound dial digit configurations.
...
1. Go back to your gateway and use the show run command to find and observe the following configuration. It’s a few lines down from the big list of IPv4 statements. This is a regular expression that matches anything in the range of 198.18.133.X subnet using port 5065.  
...
1. Press q to exit out of the configuration display mode. Issue the commands below and press enter after each one  
   ...
   debug ccsip messages  
   terminal monitor
2. Now make a call from Taylor Bard’s jabber client to the 4 digit number you just configured for the Route pattern
...
1. In a real deployment, you would also configure extra route patterns for 6 digit and +E.164 routing on UCM, but this is a Webex Calling lab - so in the interests of time we will not do this in this lab.
